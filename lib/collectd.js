...
function CollectD(options) {
    ...
    internals.client = net.connect({path: '/usr/local/var/run/collectd-unixsock'},
        function () {
            console.log('connected');
        }.bind(this));

    internals.client.on('data', function (data) {
    ...
    function _doGetVal(key, callback) {
        internals.client.write('GETVAL ' + key + '\n');
        queue.push(callback);
    }

    this.start = function(metrics, timeout) {
        var intervalTimeout = timeout || 10000;

        internals.metrics = metrics;

        if (!internals.intervalId) {
            internals.intervalId = setInterval(function () {

                metrics.forEach(function (metric) {
                        _doGetVal(metric, function (err, data) {
